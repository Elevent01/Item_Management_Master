...
def auto_display_order(db: Session) -> int:
    """Auto-generate next display_order = max existing + 1"""
...
def generate_type_code(name: str, db: Session, model, code_field: str, prefix: str = "") -> str:
    """
    Auto-generate unique code in format: XXXX-0001
    - First part : first 4 letters of name (alpha only, uppercased, padded with X if short)
      e.g. 'Asset' → 'ASST', 'Liability' → 'LIAB', 'Income' → 'INCO'
    - Second part: 4-digit sequence starting at 0001, increments until unique
      e.g. 'ASST-0001', 'ASST-0002', ...
    """
    # Strip to alpha chars only, uppercase, take first 4, pad with X if shorter than 4
    alpha = "".join(c for c in name.strip().upper() if c.isalpha())
    base = (alpha[:4]).ljust(4, "X")   # e.g. 'Asset' → 'ASSE', 'Inc' → 'INCX'
    if prefix:
    ...
    while True:
        ...
        exists = db.query(model).filter(
            getattr(model, code_field) == candidate
        ).first()
        if not exists:
            return candidate
        counter += 1
...
async def get_gl_types(
    is_active: Optional[bool] = Query(None),
    db: Session = Depends(get_db)
):
    """Get all GL Types (ASSET, LIABILITY, etc.)"""
...
async def create_gl_type(
    ...
    """Create new GL Type — type_code is always auto-generated from type_name"""
...
async def update_gl_type(
    ...
    if type_name:
        type_name = capitalize_properly(type_name)
        existing = db.query(fin_models.GLType).filter(
            fin_models.GLType.type_name == type_name,
            fin_models.GLType.id != type_id
        ).first()
        if existing:
            raise HTTPException(status_code=400, detail="GL Type name already exists")
...
async def get_gl_sub_types(
    ...
    """Get all GL Sub-Types, optionally filtered by GL Type"""
    query = db.query(fin_models.GLSubType).options(
        joinedload(fin_models.GLSubType.gl_type)
    )
    ...
    sub_types = query.order_by(
        fin_models.GLSubType.gl_type_id,
        fin_models.GLSubType.display_order,
        fin_models.GLSubType.sub_type_name
    ).all()
    ...
    for st in sub_types:
        result.append(schemas.GLSubTypeResponse(
            id=st.id,
            sub_type_code=st.sub_type_code,
            sub_type_name=st.sub_type_name,
            description=st.description,
            gl_type_id=st.gl_type_id,
...
async def create_gl_sub_type(
    ...
    """Create new GL Sub-Type — sub_type_code always auto-generated as PARENTCODE-XXXX-0001"""
    sub_type_name = capitalize_properly(sub_type_name)
    
    gl_type = db.query(fin_models.GLType).filter(
        fin_models.GLType.id == gl_type_id,
        fin_models.GLType.is_active == True
    ).first()
    if not gl_type:
        raise HTTPException(status_code=404, detail="GL Type not found or inactive")
    
    # Always auto-generate code in format: ASSE-0001-CURR-0001
    # = parent GL Type code + first 4 letters of sub-type name + 4-digit sequence
    parent_code = gl_type.type_code if gl_type.type_code else "XXXX"
    alpha = "".join(c for c in sub_type_name.strip().upper() if c.isalpha())
    sub_base = (alpha[:4]).ljust(4, "X")   # e.g. 'Current Assets' → 'CURR'
    counter = 1
    while True:
    ...
    db.add(gl_sub_type)
    db.commit()
    db.refresh(gl_sub_type)
    
    return schemas.GLSubTypeResponse(
        id=gl_sub_type.id,
        sub_type_code=gl_sub_type.sub_type_code,
        sub_type_name=gl_sub_type.sub_type_name,
        description=gl_sub_type.description,
        gl_type_id=gl_sub_type.gl_type_id,
        display_order=gl_sub_type.display_order,
        is_active=gl_sub_type.is_active,
...
async def update_gl_sub_type(
    ...
    if sub_type_name:
        sub_type_name = capitalize_properly(sub_type_name)
        existing = db.query(fin_models.GLSubType).filter(
            fin_models.GLSubType.sub_type_name == sub_type_name,
            fin_models.GLSubType.id != sub_type_id
        ).first()
        if existing:
            raise HTTPException(status_code=400, detail="GL Sub-Type name already exists")
...
# ==================== BULK CREATE: GL TYPES + SUB TYPES ====================

@router.post("/gl-bulk-create", response_model=schemas.BulkCreateResponse)
async def bulk_create_gl_types_and_subtypes(
    payload: schemas.BulkCreateRequest,
    db: Session = Depends(get_db)
):
    """
    Bulk create GL Types and GL Sub-Types in one call.
    Auto-generates codes. Skips duplicates gracefully (records as error).
    Both IFRS and Indian labels are stored simultaneously.
    """
    created_types = []
    created_sub_types = []
    errors = []
    ...
    for item in (payload.gl_types or []):
        try:
            ...
            if existing:
                errors.append({"entity": "gl_type", "name": type_name, "reason": "Name already exists"})
                continue
            ...
            created_types.append({
                "id": gl_type.id,
                "type_code": gl_type.type_code,
                "type_name": gl_type.type_name,
                "description": gl_type.description,
            })
        except Exception as e:
            errors.append({"entity": "gl_type", "name": item.type_name, "reason": str(e)})

    # ---- Process GL Sub-Types ----
    for item in (payload.gl_sub_types or []):
        try:
            sub_type_name = capitalize_properly(item.sub_type_name)
            
            # Validate GL Type exists
            gl_type = db.query(fin_models.GLType).filter(
                fin_models.GLType.id == item.gl_type_id
            ).first()
            if not gl_type:
                errors.append({"entity": "gl_sub_type", "name": sub_type_name, "reason": f"GL Type id={item.gl_type_id} not found"})
                continue
            ...
            if existing:
                errors.append({"entity": "gl_sub_type", "name": sub_type_name, "reason": "Name already exists"})
                continue
            
            # Auto-generate code — prefix with GL Type code for uniqueness + readability
            # e.g. Current Assets under Asset (ASST) → ASST-CA
            ...
            gl_sub_type = fin_models.GLSubType(
                sub_type_code=sub_type_code,
                sub_type_name=sub_type_name,
                gl_type_id=item.gl_type_id,
                description=item.description,
                display_order=item.display_order if item.display_order is not None else auto_display_order(db),
                created_by=payload.user_id
            )
            db.add(gl_sub_type)
            db.flush()
            
            created_sub_types.append({
                "id": gl_sub_type.id,
                "sub_type_code": gl_sub_type.sub_type_code,
                "sub_type_name": gl_sub_type.sub_type_name,
                "gl_type_id": gl_sub_type.gl_type_id,
    ...
    return schemas.BulkCreateResponse(
        ...
        created_sub_types=created_sub_types,
        errors=errors,
        total_types_created=len(created_types),
        total_sub_types_created=len(created_sub_types),
...
async def preview_auto_code(
    name: str = Query(...),
    entity: str = Query(..., description="'type' or 'sub_type'"),
    db: Session = Depends(get_db)
):
    """Preview the auto-generated code for a given name before saving"""
    if entity == "type":
...
async def get_gl_categories(
    ...
    """Get all GL Categories (optionally filtered by GL Type / Sub-Type)"""
    query = db.query(fin_models.GLCategory).options(
        joinedload(fin_models.GLCategory.gl_type),
        joinedload(fin_models.GLCategory.gl_sub_type)
    )
    ...
    for cat in categories:
        result.append(schemas.GLCategoryResponse(
            ...
            gl_type_id=cat.gl_type_id,
            gl_sub_type_id=cat.gl_sub_type_id,
...
async def create_gl_category(
    ...
    """Create new GL Category — category_code always auto-generated as PARENTCODE-CATX-0001"""
    ...
    gl_sub_type = None
    if gl_sub_type_id:
        gl_sub_type = db.query(fin_models.GLSubType).filter(
            fin_models.GLSubType.id == gl_sub_type_id
        ).first()
        if not gl_sub_type:
            raise HTTPException(status_code=404, detail="GL Sub-Type not found")

    # Auto-generate: parent_code-CATX-0001
    ...
    db.add(gl_category)
    db.commit()
    db.refresh(gl_category)

    return schemas.GLCategoryResponse(
        id=gl_category.id,
        category_code=gl_category.category_code,
        category_name=gl_category.category_name,
        description=gl_category.description,
        gl_type_id=gl_category.gl_type_id,
        gl_sub_type_id=gl_category.gl_sub_type_id,
        gl_type_code=gl_type.type_code,
        gl_type_name=gl_type.type_name,
        ...
        is_active=gl_category.is_active,
        created_at=gl_category.created_at
    )
...
async def update_gl_category(
    ...
    if category_name:
        category_name = capitalize_properly(category_name)
        existing = db.query(fin_models.GLCategory).filter(
            fin_models.GLCategory.category_name == category_name,
            fin_models.GLCategory.id != category_id
        ).first()
        if existing:
            raise HTTPException(status_code=400, detail="GL Category name already exists")
...
async def get_gl_sub_categories(
    ...
    """Get all GL Sub-Categories, optionally filtered"""
    query = db.query(fin_models.GLSubCategory).options(
        joinedload(fin_models.GLSubCategory.gl_type),
        joinedload(fin_models.GLSubCategory.gl_sub_type),
    ...
    for sc in items:
        result.append(schemas.GLSubCategoryResponse(
            ...
            gl_type_id=sc.gl_type_id,
            gl_sub_type_id=sc.gl_sub_type_id,
            gl_category_id=sc.gl_category_id,
            is_active=sc.is_active,
            created_at=sc.created_at,
            gl_type_code=sc.gl_type.type_code if sc.gl_type else None,
            gl_type_name=sc.gl_type.type_name if sc.gl_type else None,
            gl_sub_type_code=sc.gl_sub_type.sub_type_code if sc.gl_sub_type else None,
            gl_sub_type_name=sc.gl_sub_type.sub_type_name if sc.gl_sub_type else None,
            gl_category_code=sc.gl_category.category_code if sc.gl_category else None,
            gl_category_name=sc.gl_category.category_name if sc.gl_category else None,
        ))
    return result
...
async def create_gl_sub_category(
    ...
    """Create GL Sub-Category — code auto-generated as CATEGORY_CODE-SCXX-0001"""
    ...
    # Auto-generate: CATEGORY_CODE-SCXX-0001
    ...
    while True:
        ...
        if not db.query(fin_models.GLSubCategory).filter(
            fin_models.GLSubCategory.sub_category_code == sub_category_code
        ).first():
            break
        counter += 1

    if db.query(fin_models.GLSubCategory).filter(
        fin_models.GLSubCategory.sub_category_name == sub_category_name
    ).first():
...
async def get_user_companies_with_plants(user_id: int, db: Session = Depends(get_db)):
    """Get companies and plants accessible to user (same as Category)"""
...
async def create_gl_account(
    ...
    existing = db.query(fin_models.GLMaster).filter(
        fin_models.GLMaster.company_id == company_id,
        fin_models.GLMaster.gl_code == gl_code
    ).first()
    if existing:
    ...
    scope = "Company-wide" if not plant_id else f"Plant: {plant.plant_name if plant_id else ''}"
...
async def get_gl_accounts_by_user(
    ...
    """Get all GL Accounts accessible to user"""
    ...
    query = db.query(fin_models.GLMaster).filter(
        or_(
            and_(
                fin_models.GLMaster.company_id.in_(accessible_companies),
                fin_models.GLMaster.plant_id.is_(None)
            ),
    ...
    if plant_id:
        query = query.filter(
            or_(
                fin_models.GLMaster.plant_id == plant_id,
                fin_models.GLMaster.plant_id.is_(None)
            )
        )
    ...
    if search:
        query = query.filter(
            or_(
                fin_models.GLMaster.gl_code.ilike(f"%{search}%"),
                fin_models.GLMaster.gl_name.ilike(f"%{search}%")
            )
        )
    ...
    for gl in gl_accounts:
        child_count = db.query(func.count(fin_models.GLMaster.id)).filter(
            fin_models.GLMaster.parent_gl_id == gl.id
        ).scalar()
        ...
        result.append({
            ...
            "company": {
                "id": gl.company.id,
                "name": gl.company.company_name,
                "code": gl.company.company_code
            },
            "plant": {
                "id": gl.plant.id,
                "name": gl.plant.plant_name,
                "code": gl.plant.plant_code
            } if gl.plant else None,
            "gl_type": {
                "id": gl.gl_type.id,
                "type_code": gl.gl_type.type_code,
                "type_name": gl.gl_type.type_name
            },
            "gl_sub_type": {
                "id": gl.gl_sub_type.id,
                "sub_type_code": gl.gl_sub_type.sub_type_code,
                "sub_type_name": gl.gl_sub_type.sub_type_name
            } if gl.gl_sub_type else None,
            "gl_category": {
                "id": gl.gl_category.id,
                "category_code": gl.gl_category.category_code,
                "category_name": gl.gl_category.category_name
            } if gl.gl_category else None,
            "parent_gl": {
                "id": gl.parent_gl.id,
                "gl_code": gl.parent_gl.gl_code,
                "gl_name": gl.parent_gl.gl_name
            } if gl.parent_gl else None,
...
async def update_gl_account(
    ...
    """Update GL Account (with history tracking)"""
    ...
    if gl_name and gl_name.strip() != gl.gl_name:
        name_history = fin_models.GLNameHistory(
            gl_id=gl_id,
            old_name=gl.gl_name,
            new_name=capitalize_properly(gl_name),
            changed_by=user_id,
            reason=reason
        )
        db.add(name_history)
    ...
    if is_active is not None and is_active != gl.is_active:
        status_history = fin_models.GLStatusHistory(
            gl_id=gl_id,
            old_status=gl.is_active,
...
async def get_gl_account_history(gl_id: int, db: Session = Depends(get_db)):
    """Get GL Account change history"""
    gl = db.query(fin_models.GLMaster).filter(fin_models.GLMaster.id == gl_id).first()
    if not gl:
        raise HTTPException(status_code=404, detail="GL Account not found")
    
    name_history = db.query(fin_models.GLNameHistory).filter(
        fin_models.GLNameHistory.gl_id == gl_id
    ).order_by(fin_models.GLNameHistory.changed_at.desc()).all()
    
    status_history = db.query(fin_models.GLStatusHistory).filter(
        fin_models.GLStatusHistory.gl_id == gl_id
    ).order_by(fin_models.GLStatusHistory.changed_at.desc()).all()
    
    return {
        "gl_id": gl_id,
        "gl_code": gl.gl_code,
        "current_name": gl.gl_name,
        "current_status": gl.is_active,
        "name_changes": [{
            "old_name": h.old_name,
            "new_name": h.new_name,
        ...
        "status_changes": [{
            "old_status": "Active" if h.old_status else "Inactive",
            "new_status": "Active" if h.new_status else "Inactive",
            "changed_at": h.changed_at,
            "reason": h.reason
...
async def create_item(
    ...
    company_id: int = Form(...),
    gl_id: int = Form(...),
    is_stock: bool = Form(True),
    user_id: int = Form(...),
    db: Session = Depends(get_db)
):
    """Create Item with GL Mapping"""
    ...
    gl = db.query(fin_models.GLMaster).filter(
        fin_models.GLMaster.id == gl_id,
        fin_models.GLMaster.company_id == company_id
    ).first()
    if not gl:
        raise HTTPException(status_code=404, detail="GL Account not found or doesn't belong to company")
    
    existing = db.query(fin_models.ItemInfoMaster).filter(
        fin_models.ItemInfoMaster.company_id == company_id,
        fin_models.ItemInfoMaster.item_code == item_code
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"Item code '{item_code}' already exists")
    
    item = fin_models.ItemInfoMaster(
        company_id=company_id,
        item_code=item_code,
        item_name=item_name,
        gl_id=gl_id,
...
async def get_items_by_user(
    ...
    query = db.query(fin_models.ItemInfoMaster).filter(
        fin_models.ItemInfoMaster.company_id.in_(accessible_companies)
    ).options(
    ...
    for item in items:
        result.append({
            ...
            "gl_account": {
                "id": item.gl_account.id,
                "gl_code": item.gl_account.gl_code,
                "gl_name": item.gl_account.gl_name,
