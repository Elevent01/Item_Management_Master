...
export default function AddGLMaster() {
  ...
  const fetchGLTypes       = async () => { try { const r = await fetch(`${API_BASE}/gl-types?is_active=true`);  setGLTypes(await r.json() || []);         } catch (e) { console.error(e); } };
  const fetchGLSubTypes    = async () => { try { const r = await fetch(`${API_BASE}/gl-sub-types`);              setGLSubTypes(await r.json() || []);      } catch (e) { console.error(e); } };
  const fetchGLCategories  = async () => { try { const r = await fetch(`${API_BASE}/gl-categories?is_active=true`); setGLCategories(await r.json() || []); } catch (e) { console.error(e); } };
  const fetchGLSubCategories=async () => { try { const r = await fetch(`${API_BASE}/gl-sub-categories`);         setGLSubCategories(await r.json() || []);} catch (e) { console.error(e); } };
  const fetchGLHeads       = async () => { try { const r = await fetch(`${API_BASE}/gl-heads?is_active=true`);   setGLHeads(await r.json() || []);         } catch (e) { console.error(e); } };
